...
class Queue:
    ...
    def all(
        ...
        def to_queue(queue_key: bytes | str):
    ...
    def get_job_position(self, job_or_id: Job | str) -> int | None:
    ...
    def push_job_id(self, job_id: str, pipeline: Pipeline | None = None, at_front: bool = False):
    ...
    def enqueue_many(
        self, job_datas: Iterable[EnqueueData], pipeline: Pipeline | None = None, group_id: str | None = None
    ) -> list[Job]:
    ...
    def enqueue_job(
        self, job: Job, pipeline: Pipeline | None = None, at_front: bool = False, unique: bool = False
    ) -> Job:
    ...
    def _enqueue_sync_job(self, job: Job, pipeline: Pipeline | None = None, unique: bool = False) -> Job:
    ...
    def lmove(cls, connection: Redis, queue_key: str, timeout: int | None):
        ...
        if timeout is not None:  # blocking variant
            ...
            result: Any | None = connection.blmove(queue_key, intermediate_queue.key, timeout)
            if result is None:
    ...
    def dequeue_any(
        ...
        while True:
            ...
            job_fetch_result = cls._fetch_dequeued_job(
                connection,
                queue_key,
                job_id,
                job_class=job_class,
            ...
            if job_fetch_result is None:
                continue
            return job_fetch_result

    @classmethod
    def _fetch_dequeued_job(
        cls,
        connection: Redis,
        queue_key: str,
        job_id: str,
        ...
        """Post-pop processing shared by `dequeue_any` and the async worker:
        rebuild the Queue from its key and fetch the Job. Returns None when the
        job hash no longer exists (caller should dequeue again); attaches
        job_id/queue metadata to other fetch errors.
        """
